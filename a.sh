...
create_config_files() {
  ...
  local EPOXY_PORTS=(8080 8081 8082 8083)
  
  local CADDY_BACKENDS=""
  for port in "${EPOXY_PORTS[@]}"; do
      CADDY_BACKENDS+=" http://localhost:${port}"
  done
...
:443 {
    ...
    reverse_proxy @websockets${CADDY_BACKENDS} {
...
EOF
  ...
  local EPOXY_BASE_CONFIG
  read -r -d '' EPOXY_BASE_CONFIG <<'EOT'
[server]
bind = ["tcp", "0.0.0.0:__PORT__"]
transport = "websocket"
resolve_ipv6 = false
tcp_nodelay = true
file_raw_mode = false
...
EOT
  ...
  sudo rm -f /etc/epoxy-server/config.toml

  for port in "${EPOXY_PORTS[@]}"; do
      echo "$EPOXY_BASE_CONFIG" | sed "s/__PORT__/$port/" | sudo tee "/etc/epoxy-server/config-${port}.toml" >/dev/null
  done

  local EPOXY_APPS_CONFIG=""
  local i=1
  for port in "${EPOXY_PORTS[@]}"; do
      EPOXY_APPS_CONFIG+=$(cat <<EOF
    {
      name: "epoxy-${i}",
      script: "/usr/local/bin/epoxy-server",
      args: "/etc/epoxy-server/config-${port}.toml",
      exec_mode: "fork",
      instances: 1,
      autorestart: true,
      watch: false,
...
# NEW: Added reddns block to handle DNS requests
configure_redsocks() {
...
# NEW: Added UDP redirect rule for DNS
configure_transparent_proxy() {
    ...
    sudo iptables -t nat -A OUTPUT -m owner --uid-owner "$USER_ID" -p tcp ! -d 127.0.0.1 -j REDIRECT --to-port $REDSOCKS_PORT
    sudo iptables -t nat -A OUTPUT -m owner --uid-owner "$USER_ID" -p udp --dport 53 -j REDIRECT --to-port $REDDNS_PORT
    
    sudo iptables-save | sudo tee /etc/iptables/rules.v4 > /dev/null
}

# NEW: Added $REDDNS_PORT to the port check
check_and_configure_ports() {
    info "Checking network ports and firewall"
    local ports_to_check="80 443 3000 8080 8081 8082 8083"
    local ufw_ports_to_open="80 443 3000 8080 8081 8082 8083" 
    local allowed_processes="caddy epoxy-server bun wireproxy redsocks"

    for port in $ports_to_check 1080 $REDSOCKS_PORT $REDDNS_PORT; do 
...
if [ "$dependencies_needed" = true ]; then
  run_task "Installing missing dependencies" "Dependencies installed" '
    ...
    if ! command -v redsocks >/dev/null 2>&1; then
      sudo apt-get update -y >/dev/null 2>&1 && sudo apt-get install -y redsocks >/dev/null 2>&1
    fi
    if ! dpkg-query -l 2>/dev/null | grep -q iptables-persistent; then
      echo iptables-persistent iptables-persistent/autosave_v4 boolean true | sudo debconf-set-selections
      echo iptables-persistent iptables-persistent/autosave_v6 boolean true | sudo debconf-set-selections
      sudo apt-get update -y >/dev/null 2>&1 && sudo apt-get install -y iptables-persistent >/dev/null 2>&1
    fi
...
" | sudo tee -a /etc/wireproxy/wireproxy.conf
...
run_task "Creating Caddy, PM2, and Epoxy configs" "Configuration files created" "create_config_files"
run_task "Creating Redsocks config" "Redsocks config created" "configure_redsocks"
